...
- [ ] 6.2 启动前端 + 后端，打开流程设计器，手工验证：（本次实现未执行——当前会话没有可用的浏览器自动化工具，也没有已就绪的 MySQL + 已登录会话的联调环境，无法在不弄虚作假的前提下完成这几项手工验证，如实保留未勾选）
  - [ ] 6.2.1 指定人员：按姓名搜索多选 2 个用户，保存后重新打开节点，确认回显姓名而不是 id
  - [ ] 6.2.2 指定人员：按 11 位手机号搜索能搜到对应用户
  - [ ] 6.2.3 指定角色：输入角色名称关键字和角色编码关键字都能筛出对应角色，选中后保存/重新打开能正确回显角色名称
  - [ ] 6.2.4 指定组织负责人：角色选择控件可选可不选，行为与之前手输编码等价
  - [ ] 6.2.5 指定岗位：下拉展示岗位类型字典项名称，选中后保存/重新打开能正确回显
  - [ ] 6.2.6 发布一条引用了上述配置的流程，确认发布校验和运行时审批人解析行为与改动前一致（因为 DSL 取值语义未变）
