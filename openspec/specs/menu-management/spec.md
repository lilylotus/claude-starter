...
系统 SHALL 在资源详情弹窗中展示该资源（菜单/按钮/API）的操作历史列表：按操作发起时间降序排列，每页 5 条，支持分页；覆盖新增、编辑、启用、停用四类操作（不包含删除记录——资源被逻辑删除后其详情弹窗本身不可访问，历史列表天然不会出现删除记录）；每条历史记录展示操作时间、操作类型、操作人；点击某条记录可查看该次操作的字段级变更详情（旧值→新值）。
