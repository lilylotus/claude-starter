...
维护组织（`ORG`）、人员（`USER`）、任职（`POSITION`）、应用（`APP`）四类业务对象"可开放配置"的表字段目录（`tab_metadata_field`）：记录每个可配置字段的所属表、列名、列类型、展示名称，作为"表单字段定义"（`form-field-definition-management`）新增字段定义时的可选绑定来源。目录本身只能通过数据库迁移预置，不支持前端新增或删除，仅支持编辑展示名称与启用/停用状态。
