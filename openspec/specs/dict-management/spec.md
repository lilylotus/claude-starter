...
系统 SHALL 在字典类型详情弹窗、字典项详情弹窗中分别展示该条记录自身的操作历史列表：按操作发起时间降序排列，每页 5 条，支持分页；覆盖新增、编辑、启用、停用四类操作（不包含删除记录——记录被逻辑删除后其详情弹窗本身不可访问，历史列表天然不会出现删除记录）；每条历史记录展示操作时间、操作类型、操作人；点击某条记录可查看该次操作的字段级变更详情（旧值→新值）。两个详情弹窗每次关闭后再重新打开 SHALL 分别展示截至当前的最新操作历史，不复用上一次打开时加载的旧数据。
...
- **WHEN** 用户打开某个字典类型（或字典项）的详情弹窗查看历史后关闭，随后编辑保存该字典类型（或字典项），再次打开同一条记录的详情弹窗
