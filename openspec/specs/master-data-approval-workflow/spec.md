...
"审批历史"页面 SHALL 展示当前登录用户自己已经处理过（同意/拒绝/转办/委派/加签/退回）的历史记录分页列表，每条记录 SHALL 包含业务对象类型、节点名称、处理结果、处理意见、申请人与处理时间，支持按业务对象类型过滤；该页面 SHALL NOT 提供跳转到完整申请详情或流程图的入口（列表本身携带的信息已覆盖核心诉求，深入查看留待后续 change）。
...
- **THEN** 系统返回该用户自己已经处理过的历史记录分页列表，每条记录展示业务对象类型、节点名称、处理结果、处理意见、申请人与处理时间
