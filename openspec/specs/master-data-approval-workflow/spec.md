...
- **THEN** 详情中展示当前所处审批节点名称，以及该流程完整的节点/连线拓扑图（含尚未到达的后续节点），已经过的节点标注为已完成并直接展示对应的处理人、处理时间与意见（不需要额外点击），当前节点标注为进行中，尚未到达的节点标注为未到达
...
#### Scenario: 无审批历史查看权限的用户看不到审批历史菜单
- **WHEN** 当前登录用户的权限编码集合不包含 `ApprovalManagement:record:view`
- **THEN** 侧边导航不展示"审批历史"菜单项

#### Scenario: 查看自己的审批历史列表
- **WHEN** 拥有 `ApprovalManagement:record:view` 权限点的用户打开"审批历史"页面
- **THEN** 系统返回该用户自己已经处理过的历史记录分页列表，每条记录展示业务对象类型、操作类型、审批节点、处理结果、处理意见、申请人与处理时间

#### Scenario: 按业务对象类型过滤审批历史
- **WHEN** 用户在"审批历史"页面选择业务对象类型为"用户"进行过滤
- **THEN** 返回结果仅包含该用户处理过的、业务对象类型为"用户"的历史记录

#### Scenario: 从审批历史打开完整申请详情
- **WHEN** 用户在"审批历史"页面对一条自己已处理过的记录点击"详情"
- **THEN** 系统按该记录的 `businessId` 查询到完整申请详情并展示，内容与展示方式和"我的申请""待我审批"的申请详情弹窗一致，包含审批流程图与条件分支可读文案

#### Scenario: 无关用户直接调用单条详情接口被拒绝
- **WHEN** 某用户既不是某条申请的提交人，也从未处理过该申请关联的流程实例，也不在该申请当前所处节点的候选人范围内，直接调用 `GET /api/approval-requests/{id}` 查询该申请详情
- **THEN** 系统拒绝该次查看请求，返回无权限错误

#### Scenario: 未关联流程实例的申请按提交人或审批人判定查看权限
- **WHEN** 一条未关联工作流流程实例的申请（`processInstanceId` 为空），由该申请的提交人或最终审批人调用单条详情接口
- **THEN** 系统正常返回申请详情；由与该申请无关的其他用户调用则拒绝访问
