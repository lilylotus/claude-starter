# workflow-approval-engine Specification

## Purpose

通用多级审批引擎：以 `WorkflowService` 抽象层隔离底层 Flowable 引擎 API，配合可配置的节点审批人规则（`AssigneeResolver` 体系）、空审批人与自审场景处理策略、会签（Multi-Instance）、Reject/Return/Withdraw/Transfer/Delegate/AddSign 操作语义、审批操作幂等与任务处理越权校验、以及不依赖 Flowable 运行时表的待办/已办查询，为业务代码提供一个与具体流程模型解耦、可承载任意业务多套流程的审批基础设施。
