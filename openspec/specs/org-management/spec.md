...
更新组织导致 `parentId` 发生变化（变更了上级组织）时，系统 SHALL 重新解析该组织自身的 `orgPath`/`orgParentPath`，并级联更新其**全部**未被逻辑删除的子孙组织（不限于直属子组织）的 `orgPath`/`orgParentPath`；未变更上级组织时 `orgPath`/`orgParentPath` 保持不变。组织自身的 `name` 发生变化时，系统 SHALL 级联更新该组织**全部**未被逻辑删除的子孙组织的 `orgNamePath`。
...
- **THEN** 新组织的 `orgPath` 为 `1/2/3`、`orgNamePath` 为 `一级组织/二级组织/三级组织`、`orgParentPath` 为 `1/2`

#### Scenario: 新建顶级组织时路径字段只含自身
- **WHEN** 客户端创建一个 `parentId=0`、自身 id 为 `10`、名称为"顶级组织"的组织
- **THEN** 新组织的 `orgPath` 为 `10`、`orgNamePath` 为"顶级组织"、`orgParentPath` 为空

#### Scenario: 变更上级组织时级联更新全部子孙的路径字段
- **WHEN** 组织 C（`orgPath=1/2/3`）拥有子孙 D（`orgPath=1/2/3/4`）与 E（`orgPath=1/2/3/4/5`），并将 C 的上级改为组织 6（`orgPath=1/6`）
- **THEN** C、D、E 的 `orgPath` 分别变为 `1/6/3`、`1/6/3/4`、`1/6/3/4/5`，其 `orgParentPath` 同步更新

#### Scenario: 组织改名时级联更新全部子孙的名称路径
- **WHEN** 组织 B（`orgNamePath=一级组织/二级组织`）改名为"二级组织新"
- **THEN** B 及其全部子孙的 `orgNamePath` 中对应路径段同步替换，其他名称段保持不变

#### Scenario: 未变更上级组织与名称时路径字段保持不变
- **WHEN** 更新组织时 `parentId` 与 `name` 均未变化
- **THEN** 该组织及其子孙的三个路径字段均保持不变

#### Scenario: 请求体携带路径字段时被忽略
- **WHEN** 新增或更新请求体携带 `orgPath`、`orgNamePath` 或 `orgParentPath`
- **THEN** 系统忽略客户端取值并按组织结构自行派生
...
#### Scenario: 祖先迁移递增全部子孙版本
- **WHEN** 组织 C 迁移父组织并导致其多层子孙的 orgPath 变化
- **THEN** C 及全部受影响子孙的 version 各自原子递增 1，updateTime 更新，事件携带递增后的版本
