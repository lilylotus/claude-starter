...
- `.Codex/agents/` — 项目专属的 subagent，把下面的约定固化了下来；遇到匹配的任务时
  优先委托给对应 agent，而不是在主对话里另起一套做法。

### 后端约定（`.Codex/agents/springboot-backend-dev.md`）
...
### OpenSpec 文档同步（`.Codex/agents/openspec-doc-sync.md`）
...
注意：
1. 不要添加 Co-Authored-By: Codex Sonnet 5 <noreply@anthropic.com> 提示
2. 若是功能描述过多可以精简提交说明
