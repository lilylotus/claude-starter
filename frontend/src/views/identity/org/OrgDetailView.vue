...
// 详情页展示当前启用状态的全部自定义字段定义（bizType=ORG），不受 showInList/
...
// OrgRow 的既有约定一致，只在需要动态取值处就地转换），故此处显式转换一次
    ...
    <template v-else>
      <section class="org-detail__panel">
        <el-descriptions v-loading="loading" :column="2" border>
          ...
          <el-descriptions-item
            v-for="item in orgFields.schema.filter((f) => f.columnName.startsWith('ext'))"
            :key="item.fieldCode"
            :label="item.fieldName"
          >
            <span v-if="item.controlType === FORM_FIELD_CONTROL_TYPE_DICT">
              {{ orgFields.dictOptionLabel(item, extFieldValue(item)) || '-' }}
            </span>
            <span v-else-if="item.controlType === FORM_FIELD_CONTROL_TYPE_MULTI_DICT">
              {{ orgFields.dictOptionLabels(item, extFieldValue(item)) || '-' }}
            </span>
