...
// 权限点详情独立页面：原 PermissionManagementView.vue 内的只读详情弹窗迁移至此，
...
import { PERMISSION_STATUS_ENABLED, type PermissionRow } from '@/types/permission'
...
async function fetchDetail() {
  ...
  loadError.value = ''
  try {
    detailData.value = await permissionApi.getPermissionById(permissionId.value)
  } catch (error) {
    loadError.value = error instanceof Error ? error.message : '加载权限详情失败'
  } finally {
    loading.value = false
  }
}
...
// 显式跳回权限点列表路由，而不是 router.back()：详情页可能是直接通过 URL/刷新进入的，
// 浏览器历史栈在这种场景下可能为空或指向无关页面
function goBack() {
  router.push({ name: 'permission-points' })
}
</script>
...
<template>
  <div class="permission-detail">
    <header class="permission-detail__header">
      <el-button link :icon="ArrowLeft" class="permission-detail__back" @click="goBack">返回</el-button>
      <h2 class="permission-detail__title">权限详情</h2>
    </header>

    <el-alert
      v-if="loadError"
      class="permission-detail__error"
      type="error"
      :title="loadError"
      show-icon
      :closable="false"
    ...
    <template v-else>
      <section class="permission-detail__panel">
...
.permission-detail__title {
  font-size: 16px;
  color: var(--color-ink);
  margin: 0;
}

.permission-detail__error {
  margin-bottom: 4px;
}
