...
/**
 * 拉取日志分页查询的筛选参数，除 {@link #appRefId} 外均可选（add-app-sync-notify-pull-logs
 * change design.md Decision 3）。
 */
@Getter
@Setter
@Schema(description = "拉取日志分页查询参数")
public class AppPullRecordQueryRequest {

    /** 应用 id（{@code tab_app.id}），精确匹配，由 controller 从路径参数回填，不接受外部输入。 */
    private Long appRefId;

    /** 拉取发起时间范围起点（含），可选。 */
    @Schema(description = "拉取发起时间范围起点（含）")
    private LocalDateTime startTime;

    /** 拉取发起时间范围终点（含），可选。 */
    @Schema(description = "拉取发起时间范围终点（含）")
