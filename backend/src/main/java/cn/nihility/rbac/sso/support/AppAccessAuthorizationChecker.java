 ...
 * {@link AppAccessEffectivePermissionService#isAuthorized(Long, Long, String, String)}
...
public class AppAccessAuthorizationChecker {
     ...
     * {@code catch (SsoProtocolException e)} 分支）。
     *
     * @param userId    用户 id
    ...
    public void assertAuthorized(Long userId, Long appId, String clientIp, String userAgent) {
        if (!appAccessEffectivePermissionService.isAuthorized(userId, appId, clientIp, userAgent)) {
            throw new SsoProtocolException("当前用户无权访问该应用");
        }
    }
}
