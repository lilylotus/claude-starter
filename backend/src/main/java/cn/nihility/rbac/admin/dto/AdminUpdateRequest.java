...
/**
 * 更新管理员的请求参数，状态字段不通过本接口修改；角色列表与管辖组织范围按整体替换语义
 * 处理（更新后的关联集合与本次请求携带的集合完全一致）。
 */
@Getter
@Setter
@Schema(description = "更新管理员请求参数")
public class AdminUpdateRequest {
    ...
    /** 关联角色 id 列表，可为空数组，覆盖既有关联集合，不校验其指向的角色是否存在且启用。 */
    @Schema(description = "关联角色 id 列表，可为空数组，覆盖既有关联集合")
    private List<Long> roleIds = new ArrayList<>();

    /** 管辖组织范围列表，可为空数组，覆盖既有关联集合。 */
    @Valid
    @Schema(description = "管辖组织范围列表，可为空数组，覆盖既有关联集合")
